...
public class FHNetworkManager : MonoBehaviour
{
		...
		public static void Init (FHClient c)
		{
				...
				aClient.RegisterHandler (MessagesFromServer.ChatPublic, typeof(M_S_Chat));
				aClient.RegisterHandler (MessagesFromServer.SysPosition, typeof(M_S_SyncPosition));
				aClient.RegisterHandler (MessagesFromServer.SysPosition, typeof(M_S_LeaveRoom));
...
//				mRoom.Send (aClient);
		...
		public static void JoinFishRoom (int idRoom, string username, int position)
		{
				M_C_JoinFishRoom mRoom = new M_C_JoinFishRoom (idRoom, username, position);
				mRoom.Send (aClient);
		}
