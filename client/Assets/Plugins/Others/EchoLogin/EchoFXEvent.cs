...
public class EchoFXEvent : EchoLinkedList
{
	...
	static int 				poolListID;  // make one of this for each class extended from EchoLinkedList
...
//&-----------------------------------------------------------------------------
	public static EchoFXEvent CellAnimate_UVSet ( EchoGameObject iego, int istartcell, int icellcount, float itimepercell, int ireps = 1 )
	{
		EchoFXEvent efx;
	...
	public override void ProcessInUpdate()
	{
		...
		switch ( eventType )
		{
			...
			case EchoET.SHAKE_OBJECT:
				if ( fxPer <= 1.0 )
                {
//					fxPer = Mathf.Sin ( fxPer * Mathf.PI );
			...
			case EchoET.CELLANIM_ECHOUV:
				if ( fxPer >= 1.0 )
				{
					fxNum1 = ( fxAnimCurCell % fxIArg1 ) * fxArg1;
					fxNum2 = 1.0f - ( fxAnimCurCell / fxIArg1 ) * fxArg2;
			...
			case EchoET.SHIELD_HIT:
				...
				fxNum1 = Mathf.Lerp ( -0.86f, -0.82f, Random.Range ( 0.0f, 1.0f ) );
				...
				if ( fxPer >= 1.0f )
				{
					ego.ShaderSet_echoHitMix ( fxShieldHitID, -1.1f );
