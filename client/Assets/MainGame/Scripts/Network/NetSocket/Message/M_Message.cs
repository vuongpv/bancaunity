...
namespace FHNetSocket
{
    ...
    public abstract class M_Message
    {
        ...
        public M_Message(string rawMessage)
            : this()
        {
		...
		public static M_Message Factory(string rawMessage)
		{
			...
			if (reMessageType.IsMatch(rawMessage))
			{
				...
				{
                    case '0':
                        return M_DisconnectMessage.Deserialize(rawMessage);
                    case '1':
                        return M_ConnectMessage.Deserialize(rawMessage);
                    case '2':
                        return new M_Heartbeat();
                    case '3':
                        return M_TextMessage.Deserialize(rawMessage);
                    case '4':
                        return M_JSONMessage.Deserialize(rawMessage);
                    case '5':
                        return M_EventMessage.Deserialize(rawMessage);
                    case '6':
                        return M_AckMessage.Deserialize(rawMessage);
                    case '7':
                        return M_ErrorMessage.Deserialize(rawMessage);
                    case '8':
                        return new M_NoopMessage();
					default:
						Debug.LogError(string.Format("Message.Factory undetermined message: {0}", rawMessage));
						return null;
				}
			}
			else
			{
				Debug.LogError(string.Format("Message.Factory did not find matching message type: {0}", rawMessage));
				return null;
			}
		}
    }
