...
/// </remarks>
[ExecuteInEditMode]
public class SpriteCommon : SpriteBase
{
	/// <summary>
	...
	*/
	...
	static public SpriteCommon Create(string name, Vector3 pos)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
		return (SpriteCommon)go.AddComponent(typeof(SpriteCommon));
	}
	...
	/// <returns>Returns a reference to the component.</returns>
	static public SpriteCommon Create(string name, Vector3 pos, Quaternion rotation)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
...
public class SpriteMirror : SpriteRootMirror
{
	...
	public override void Mirror(SpriteRoot s)
	{
		...
		lowerLeftPixel = ((SpriteCommon)s).lowerLeftPixel;
		pixelDimensions = ((SpriteCommon)s).pixelDimensions;
	}
	...
	public override bool DidChange(SpriteRoot s)
	{
		if (base.DidChange(s))
			return true;
		if (((SpriteCommon)s).lowerLeftPixel != lowerLeftPixel)
		{
			s.uvsInitialized = false;
			return true;
		}
		if (((SpriteCommon)s).pixelDimensions != pixelDimensions)
		{
			s.uvsInitialized = false;
			return true;
		}
