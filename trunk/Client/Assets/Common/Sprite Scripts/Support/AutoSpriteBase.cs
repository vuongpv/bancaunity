...
public class TextureAnim
{
		...
		[HideInInspector]
		public CSpriteFrame[]
				spriteFrames;		// Array that parallels the Texture2D frameGUIDs array and holds the post-packing atlas UV coords for the given frame
		...
		public override bool StepAnim (float time)
		{
				...
				*/
				...
				if (crossfadeFrames) {
						...
						uvs2 [0].x = nextUV.xMin;
						uvs2 [0].y = nextUV.yMax;
						uvs2 [1].x = nextUV.xMin;
						uvs2 [1].y = nextUV.yMin;
						uvs2 [2].x = nextUV.xMax;
						uvs2 [2].y = nextUV.yMin;
						uvs2 [3].x = nextUV.xMax;
						uvs2 [3].y = nextUV.yMax;
		...
		*/
		...
		public virtual Material GetPackedMaterial (out string errString)
		{
				...
				if (spriteMesh == null) {
						...
						} else { // Else get the material from the sprite's renderer	 // as this is probably a prefab and that's why it
								// doesn't have a mesh:
