...
public class EZLinkedList<T> where T : IEZLinkedListItem<T>
{
		...
		public void Remove (T item)
		{
				...
				if (head.Equals (item)) {
						...
						if (item.next != null) {	// Connect both sides:
								if (item.prev != null)
...
public abstract class SpriteRoot : MonoBehaviour, IEZLinkedListItem<ISpriteAnimatable>, IUseCamera
{
	...
	*/
		...
		[HideInInspector]
		public bool
				billboarded = false;			// Is the sprite to be billboarded? (not currently supported)
		[System.NonSerialized]
		public bool
				isClone = false;				// Set this to true when the sprite has been instantiated from another sprite in the scene.
		[System.NonSerialized]
		public bool
				uvsInitialized = false;			// This is set to true when the sprite's UVs have been initialized.
		...
		[HideInInspector]
		public Vector2
				pixelsPerUV;				// The number of pixels in both axes per UV unit
		...
		if (StringEndsWith(name, "(Clone)"))
				...
				if (!managed) {
						...
						if (manager == null && !string.IsNullOrEmpty (managerName) && SpriteManager.managers.ContainsKey (managerName)) {
								manager = SpriteManager.managers [managerName];
						}
...
*/
		...
		{
				if (!s.managed) {	// Don't let it be changed to underscore
