...
public interface TransitionActionDelegate
{
		void Begin ();
		bool IsFinished ();
		void End ();
...
public class TransitionManager : MonoBehaviour
{
		...
		public UISprite fadingTexture = null;
		...
		private TransitionActionDelegate actionDelegate;
		...
		fadingTexture.layer = gameObject.layer;
		...
		private void DoSplashScreens ()
		{
				...
				fadingIn = true;
#if UNITY_4_0
		fadingTexture.gameObject.SetActive(true);
#elif UNITY_3_5
		fadingTexture.gameObject.SetActiveRecursively(true);
#endif
		}
				...
				if (presentingSS || showingStory) {
						...
						if (fadingIn) {
								...
								if (color.a <= 0f) {
										...
										waiting = true;
#if UNITY_4_0
					fadingTexture.gameObject.SetActive(false);
#elif UNITY_3_5
					fadingTexture.gameObject.SetActiveRecursively(false);
#endif
								}
								fadingTexture.color = color;
						} else if (fadingOut) {
								color.a += Time.deltaTime / (SplashScreenFadingTime * 2f);
								if (color.a >= 0.5f) {
										color.a = 0.5f;
		...
		public void DoAction (TransitionActionDelegate action, bool fadeOutFirst)
		{
				actionDelegate = action;

				LoadScene (null, fadeOutFirst);
		}
