...
namespace SocketIOClient
{
	public class CookieWebClient : WebClient
	{
		...
		protected override WebRequest GetWebRequest(Uri address)
		{
			...
			{
				//UnityEngine.Debug.Log("iniiiiiiiiiiiiiiiiiiit cooooooooooookkkkkkkkk");
		...
		{
			UnityEngine.Debug.Log("coooookkkkkkk " + cookieContainer.Count);
		}*/
	}
	/// <summary>
	...
	public class Client : IDisposable, SocketIOClient.IClient
	{
				...
				if (!(this.ReadyState == WebSocketState.Connecting || this.ReadyState == WebSocketState.Open))
				{
					try
					{
						...
						else
						{
							...
							CookieCollection cookies = CookieWebClient.cookieContainer.GetCookies(new Uri(string.Format("{0}://{1}:{2}", uri.Scheme, uri.Host, uri.Port)));
							List<KeyValuePair<string, string>> addCookies = new List<KeyValuePair<string, string>>();
							for(int i = 0; i < cookies.Count; i++)
							{

								Cookie c = cookies[i];
								//UnityEngine.Debug.Log("coookkkkk name: " + c.Name + " value: " + c.Value);
								addCookies.Add(new KeyValuePair<string, string>(c.Name, c.Value));
							}				
							...
							this.wsClient = new WebSocket(
								...
								addCookies,
								this.socketVersion);
							//kakapro change true->false: fix loi lau lau mat packet
		...
		{
			//UnityEngine.Debug.LogWarning("send " + msg.Encoded);
		...
		{
			//UnityEngine.Debug.Log("reveiced mess " + msg.RawMessage);
			bool skip = false;
		...
		{
			UnityEngine.Debug.LogWarning("On Open " + HandShake.HeartbeatInterval);
			//auto s end heartbeat to server
		...
		protected void OnErrorEvent(object sender, ErrorEventArgs e)
		{
			...
			UnityEngine.Debug.LogError(string.Format("Error Event: {0}\r\n\t{1}", e.Message, e.Exception));
		}
		protected void OnSocketConnectionClosedEvent(object sender, EventArgs e)
		{
			...
			UnityEngine.Debug.LogError("SocketConnectionClosedEvent");
		}
		...
		{
			//if(state == null)
			//	UnityEngine.Debug.LogWarning("Receive heartbeat");
			//UnityEngine.Debug.LogWarning("Send heartbeat");
			if (this.ReadyState == WebSocketState.Open)
			{
				...
				{
					UnityEngine.Debug.LogError(string.Format("OnHeartBeatTimerCallback Error Event: {0}\r\n\t{1}", ex.Message, ex.InnerException));
			...
			while (this.outboundQueue != null)
			{
				if (this.ReadyState == WebSocketState.Open)
				{
					...
					try
					{
						...
						{
							//UnityEngine.Debug.LogWarning("dequeuOutboundMessages " + msgString);
				...
				{
					//UnityEngine.Debug.LogError("dequeuOutboundMessages Wait 2s");
		...
		protected virtual void Dispose(bool disposing)
		{
			if (disposing)
			{
				...
				UnityEngine.Debug.LogWarning("Dispose Client");
				this.Close();
